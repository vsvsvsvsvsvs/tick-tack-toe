...
namespace
{
    ...
    int evaluateMove(char board[10][10], int x, int y, char symbol)
    {
        ...
        for (auto& d : dirs)
        {
            ...
            {
                if (openEnds == 2) total += 100000;
                else if (openEnds == 1) total += 10000;
            }
            else if (count == 3)
            {
                if (openEnds == 2) total += 8000;
                else if (openEnds == 1) total += 500;
            }
            else if (count == 2)
            {
                if (openEnds == 2) total += 200;
                else if (openEnds == 1) total += 50;
            }
        }
        ...
        for (int i = 0; i < 10; i++)
            for (int j = 0; j < 10; j++)
            {
                if (board[i][j] == 'O')
                    ...
                    score -= evaluateMove(board,i,j,'X') * 1.5;
            }
...
QPair<int,int> Bot::alphaBetaMove(char board[SIZE][SIZE])
{
    ...
    struct MS { QPair<int,int> m; int s; };
    QVector<MS> ordered;
